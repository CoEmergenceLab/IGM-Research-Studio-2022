...
void loadData() {
  ...
  if (touch>0 ){
    ...
    if(touch != prev) {
      shape = new Shape[int(touch)];
      for (int i=0; i< touch; i++) {
        shape[i] = new Shape(random(width), random(height));
      }
      fill(0, 255, 0, 204);
      prev = touch;
      text("Number of Times Touched: " + touch, width*0.8, height*0.05);
...
void oscEvent(OscMessage theOscMessage) {
    ...
    if (theOscMessage.checkAddrPattern("/sentiment") == true) {
      ...
      if (theOscMessage.checkTypetag("s")) {
        ...
        //Add recieved data to compile arrayList
